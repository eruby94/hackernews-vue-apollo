...
export default {
  ...
  methods: {
    ...
    updateStoreAfterVote(store, createVote, linkId) {
      // 1 Read the current state of the cached data from the store
      const data = store.readQuery({
        query: ALL_LINKS_QUERY
      })

      // 2 Retrieve the link that the user just voted for from that list & manipulate vote count
      const votedLink = data.allLinks.find(link => link.id === linkId)
      votedLink.votes = createVote.link.votes

      // 3 Take the modified data and write back to the store
